...
$(".filter-item input").change(function () {
    let checkedInputs = $(".filter-item input:checked").length
    console.log(checkedInputs)
    if(checkedInputs > 0){
        $("._selected-filter-count").html(checkedInputs)
        $(".m-filter").addClass("_selected")
    } else{
        $(".m-filter").removeClass("_selected")
    }
})
...
modalTogglers.forEach(el => {
    el.onclick = function () {
        ...
        if(modalId === "reviewModal"){
            let img = el.querySelector(".avatar img").getAttribute("src")
            let name = el.querySelector("._name").innerHTML
            let prof = el.querySelector("._prof").innerHTML
            let text = el.querySelector("._text").innerHTML

            $(".review-modal .review-card__name").html(name)
            $(".review-modal .review-card__prof").html(prof)
            $(".review-modal__text").html(text)
            $(".review-modal .avatar img").attr("src",img)
        }
...
$("._show-project-modal").click(function () {
    $(this).parents(".project-group").addClass("_open")
    $("body").addClass("fixed-body")
})

$("._hide-project-modal").click(function () {
    $(this).parents(".project-group").removeClass("_open")
    $("body").removeClass("fixed-body")
})
